...
function RecetaProductoMapModal({ open, onClose, receta, onChanged }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      const payload = {
        ...
        // Ya no enviamos vencimiento_base; el backend asume PRODUCCION
  ...
  async function remove(row) {
    if (!window.confirm(`Eliminar mapeo de ${row.producto?.nombre || `#${row.producto_id}`} ?`)) return;
    try {
      setLoading(true);
      ...
      <form onSubmit={submit} style={{ marginTop: 12 }}>
        <div className="form-grid">
          <div>
            ...
            <select name="producto_id" value={form.producto_id} onChange={handleChange} required disabled={isEdit}>
              <option value="">— Seleccione —</option>
              {productos.map((p) => (
                <option key={p.id} value={p.id}>{p.nombre}</option>
              ))}
            </select>
            {loadingProductos && <div className="muted" style={{ marginTop: 4 }}>Cargando productos…</div>}
          </div>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 12 }}>
          {isEdit && (
            <button type="button" className="btn-outline" onClick={cancelEdit} style={{ width: "auto" }}>
              Cancelar edición
            </button>
          )}
          <button className="btn-primary" disabled={!canSubmit || loading} style={{ width: "auto" }}>
              ...
              <tr key={row.id}>
                ...
                <td>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    <button className="btn-outline" onClick={() => startEdit(row)} style={{ width: "auto" }}>
                      ✏️ Editar
                    </button>
                    <button className="btn-danger-outline" onClick={() => remove(row)} style={{ width: "auto" }}>
                      🗑️ Eliminar
                    </button>
                  </div>
                </td>
...
function RecetaForm({ initial = emptyReceta, onSubmit, submitting, categorias }) {
  ...
  const canSubmit =
    ...
    (!form.rendimiento_por_batch ||
      (Number(form.rendimiento_por_batch) > 0 && !Number.isNaN(Number(form.rendimiento_por_batch))));
  ...
  const catOpts = Array.isArray(categorias) ? categorias : [];
...
function IngredientesManager({ receta, open, onClose }) {
  ...
  const opcionesMp = materias.filter((m) => !usedMpIds.has(String(m.id)));
...
export default function Recetas() {
  ...
  async function toggleEstado(id, estadoActual) {
    try {
      ...
      setToast({ type: "success", message: data?.estado ? "Receta activada" : "Receta desactivada" });
      await load();
    } catch {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((r) => {
      ...
      const status =
        estado === "all" || (estado === "active" && r.estado) || (estado === "inactive" && !r.estado);
  ...
  return (
    <div className="page">
      <div className="card">
        ...
        <div style={{ marginTop: 12 }}>
          <table className="table">
            ...
            <tbody>
              {loading && <tr><td colSpan={7} style={{ padding: 14 }}>Cargando…</td></tr>}
              {!loading && sorted.length === 0 && (
                <tr><td colSpan={7} style={{ padding: 14, textAlign: "center" }}>Sin resultados</td></tr>
              )}
              {!loading &&
                sorted.map((r) => (
                  <tr key={r.id}>
      ...
      <RecetaProductoMapModal
        ...
        onChanged={() => load()} // opcional: refresca la lista
      />
      ...
      <Confirm
        ...
        message={
          toDeleteHard ? `Esta acción eliminará la receta "${toDeleteHard.nombre}" de forma permanente.` : ""
        }
        onCancel={() => {
          setConfirmHardOpen(false);
          setToDeleteHard(null);
        ...
        extra={
          <div
            className="muted"
            style={{ background: "#fff2f0", border: "1px solid #ffccc7", padding: 8, borderRadius: 8 }}
          >
