...
export default function Produccion() {
  ...
  // Horas (opcionales, pero si envías una debes enviar ambas)
  ...
  // NUEVO: código de lote manual (opcional, si vacío se usa YYYYMMDD)
  ...
  const canRegister =
    calc?.ok === true && lastSig === `${recetaId}|${cantidad}` && !calcLoading;
  ...
  async function doRegistrar() {
    ...
    try {
      // fechas/horas: enviamos ISO si vienen
      const payload = {
        receta_id: Number(recetaId),
      ...
      // limpiar cálculo y (opcional) campos
      setCalc(null);
      setLastSig("");
      setObservacion("");
      // lote manual lo limpiamos a propósito para evitar reuso accidental
      setLoteCodigo("");
    } catch (e) {
      setToast({
        type: "error",
  ...
  const expectedUnits = useMemo(() => {
    // Salida estimada = #batches × rendimiento_por_batch
    return batches * rpb;
  }, [batches, rpb]);
    ...
    <div className="page">
      <div className="card">
        ...
        <div className="muted" style={{ marginBottom: 12 }}>
          Selecciona categoría y receta, calcula los insumos (sin descontar), revisa,
          y luego registra para descontar por FIFO. Puedes indicar hora de inicio / fin.
          Si el producto mapeado requiere congelación previa, el lote entrará en
        ...
        <div className="form-grid">
          <div>
            ...
            >
              <option value="">
                {loadingCats ? "Cargando..." : "Todas las categorías"}
              </option>
              {categorias.map((c) => (
          ...
          <div>
            ...
            >
              <option value="">
                {loadingRecetas ? "Cargando..." : "Seleccione receta"}
              </option>
              {recetas.map((r) => (
          ...
          <div style={{ display: "flex", alignItems: "end", gap: 8 }}>
            <button
              ...
              onClick={doCalcular}
              disabled={
                calcLoading || !recetaId || !cantidad || Number(cantidad) <= 0
              }
            >
          ...
          <div className="card" style={{ marginTop: 16 }}>
            ...
            >
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
                ...
                <span>
                  {fmtDec(batches)} batch(es) × {fmtDec(rpb)} ={" "}
                  <b>{fmtDec(expectedUnits)}</b>{" "}
              ...
              {!selectedReceta?.presentacion_id && (
                <div className="muted" style={{ marginTop: 4 }}>
                  Esta receta no tiene una presentación asociada; la salida se muestra en “unidades”.
                </div>
              )}
            </div>
            ...
            {/* Plan de lotes por ingrediente (SIN columna "Usar") */}
            {calc.detalles?.map((d) => {
